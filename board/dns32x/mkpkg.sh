...
if test "$(realpath $(dirname $0))" != "$(realpath board/dns32x)"; then
...
case "$1" in
	-rm)
		...
		#if ! grep -q ^BR2_PACKAGE_$(echo $2 | tr '[:lower:]-' '[:upper:]_') $CONFIGFILE; then
		#	echo package $2 not configured
		#	exit 1
		#fi
		rm -f $BLDDIR/images/$BOARD/autotools-stamps/$2_*
		cd $ROOTFSDIR
		# remove files first
...
if test "$force" != "y"; then
	if test "$pkg" = "gdb" -a -n "$(grep '^BR2_PACKAGE_GDB=y' $CONFIGFILE)"; then # special gdb case
		PKGDIR=toolchain/gdb
		version=$(sed -n 's/^BR2_GDB_VERSION="\(.*\)"/\1/p' $CONFIGFILE)
	else
		PKGMK=$(find $CDIR/package -name $pkg.mk)
		if test -z "$PKGMK"; then
			#echo Package $pkg not found, is it a sub-package?

			if grep -q "^BR2_PACKAGE_$PKG=y" $CONFIGFILE; then
