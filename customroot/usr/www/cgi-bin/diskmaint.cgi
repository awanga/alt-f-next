...
cat<<-EOF
	<form action="/cgi-bin/diskmaint_proc.cgi" method="post">
	...
	for k in clean format convert; do
		if test -f /tmp/$k-$part; then
			if test -d /proc/$(cat /tmp/$k-$part.pid); then
				working="yes"
				echo "<td></td><td><font color=RED>${k}ing</font></td></tr>"
			else
				rm -f /tmp/$k-$part*
			fi
		fi
...
#			/'$mdev'/ {printf ("%.4s %.4s %.4s", $5, $6, $7)}')"
		...
		if test "$action" != "idle"; then
			...
			cat<<-EOF
				...
				<td><select name=rdev_$mdev>$raid_devs</select></td>
				<td><select name=rops_$mdev>$raid_ops</select></td>
