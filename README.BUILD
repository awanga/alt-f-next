...
development tree, unstable, or a stable release.
...
In $BLDDIR/project_build_arm/dns323/root/ you will find the directory structure
used to create the root filesystem image, "rootfs.arm.ext2".
That directory was first populated by buildroot, using first the contents of target/generic/target_busybox_skeleton as a template, and then populating it
with the configured packages; after that the directory is processed by the 
"configfs" script, that copied the contents of "customroot" to it and also
...
"reloaded" to boot the new kernel.
The kernel cmdline is simply "console=ttyS0,115200". You can use other kernel recognized options, and also some Alt-F recognized options such as "initdbg",
that opens a shell for you on the serial console before "init" being run and "nodef", disabling the reading of flash-saved default settings.
...
or "lzma", that crates a lzma compressed cpio initramfs, still smaller in size
but slower to boot.

If you are already using Alt-F, you can copy zImage and rootfs.arm.cpio-sq.lzma (or
rootfs.arm.cpio.gz or rootfs.arm.cpio.lzma) to the box  root home directory,
 "/root" directory and issue a "reboot" command.
The /etc/init.d/rcE script will recognize these files and will do a kexec after
doing all other cleanup, thus reloading a new kernel and initramfs without
...
READ THE BIG RED WARNING BEFORE DOING IT. YOU HAVE BEEN WARNED.
...
should do a second or a third one :-/, at least in the first build or when
...
$BLDDIR/build_arm, and a) remove all files in $BLDDIR/project_build_arm/dns323/autotools-stamps/ with the package name, or,
