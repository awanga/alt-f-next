...
$(BUSYBOX_DIR)/busybox: $(BUSYBOX_DIR)/.config
	$(MAKE) CC=$(TARGET_CC) CROSS_COMPILE="$(TARGET_CROSS)" \
		...
		ARCH=$(KERNEL_ARCH) \
		EXTRA_CFLAGS="$(TARGET_CFLAGS) $(BR2_PACKAGE_BUSYBOX_OPTIM)" -C $(BUSYBOX_DIR)
...
ifeq ($(BR2_PACKAGE_BUSYBOX_FULLINSTALL),y)
	$(MAKE) CC=$(TARGET_CC) CROSS_COMPILE="$(TARGET_CROSS)" \
		...
		ARCH=$(KERNEL_ARCH) \
		EXTRA_CFLAGS="$(TARGET_CFLAGS) $(BR2_PACKAGE_BUSYBOX_OPTIM)" -C $(BUSYBOX_DIR) install
else
...
busybox-configure: host-sed $(BUILD_DIR) $(BUSYBOX_DIR)/.config
