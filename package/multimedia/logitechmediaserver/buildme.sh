...
# Cross compiling, don't require modules to pass tests
RUN_TESTS=0
USE_HINTS=1
...
if test -n "$CROSS"; then
	CROSS_OPTS="--enable-static --target=$CROSS --host=$CROSS"
	CFLAGS="$CFLAGS $FLAGS"
...
# CC="$CC" CFLAGS="$CFLAGS" CCFLAGS="$CCFLAGS" CPPFLAGS="$CPPFLAGS" CXX="$CXX" CXXFLAGS="$CXXFLAGS" LD="$LD" LDFLAGS="$LDFLAGS" LDDLFLAGS="$LDDLFLAGS" LD_RUN_PATH="$LD_RUN_PATH"
        $PERL_BIN Makefile.PL INSTALL_BASE=$PERL_BASE CC="$CC" CCFLAGS="$CCFLAGS" LD="$CC" LDFLAGS="$LDFLAGS" LDDLFLAGS="$LDDLFLAGS" PERL_SRC="$PERL_SRC" $2
        if [ $RUN_TESTS -eq 1 ]; then
            make test
        else
			make CC="$CC" CFLAGS="$CFLAGS" CCFLAGS="$CCFLAGS" CPPFLAGS="$CPPFLAGS" CXX="$CXX" CXXFLAGS="$CXXFLAGS" LD="$CC" LDFLAGS="$LDFLAGS" LDDLFLAGS="$LDDLFLAGS" LD_RUN_PATH="$LD_RUN_PATH"
        fi
        if [ $? != 0 ]; then
            if [ $RUN_TESTS -eq 1 ]; then
        ...
        make CC="$CC" CFLAGS="$CFLAGS" CCFLAGS="$CCFLAGS" CPPFLAGS="$CPPFLAGS" CXX="$CXX" CXXFLAGS="$CXXFLAGS" LD="$CC" LDFLAGS="$LDFLAGS" LDDLFLAGS="$LDDLFLAGS" LD_RUN_PATH="$LD_RUN_PATH"  install
        #make clean
    fi
...
function build_all {
    ...
    #build DBI
#   build DBD::mysql
	build_host_module DBI-1.616
    build DBD::SQLite
	build DBI # build target DBI
    build Digest::SHA1
    build EV
    build Encode::Detect
    build HTML::Parser
...
#             RUN_TESTS=1
            ...
            if [ ! -f build/lib/libicudata_s.a ]; then
				...
				MLDFLAGS="--sysroot=/home/jcard/Alt-F/alt-f/build/build_arm/staging_dir"
                CC="$(echo $CC | cut -d' ' -f1)" CFLAGS="$CFLAGS $ICUFLAGS" CXX="$(echo $CXX | cut -d' ' -f1)" CXXFLAGS="$CXXFLAGS $ICUFLAGS" LDFLAGS="$MLDFLAGS $FLAGS $OSX_ARCH $OSX_FLAGS" \
					./configure --prefix=$BUILD --enable-static --with-data-packaging=archive --with-cross-build="$ICU_HOST_DIR" --disable-shared $CROSS_OPTS 
                $MAKE
                if [ $? != 0 ]; then
                    echo "make failed"
                    exit $?
			...
			#sed -i '/DBI->VERSION < $DBI_required/,+4d' Makefile.PL
            cp -Rv ../hints .
...
# 			$PERL_BIN Build --makefile_env_macros 1 --config cc="$CC" --config ccflags="$CCFLAGS" --config ld="$LD" --config lddlflags="$LDDLFLAGS"
# 			$PERL_BIN Build --makefile_env_macros 1 install 
            ;;
        ...
        DBD::mysql)
            ...
            cd mysql-5.1.37
            CC=gcc CXX=gcc \
            CFLAGS=" -fno-omit-frame-pointer $FLAGS $OSX_ARCH $OSX_FLAGS" \
            CXXFLAGS=" -fno-omit-frame-pointer -felide-constructors -fno-exceptions -fno-rtti $FLAGS $OSX_ARCH $OSX_FLAGS" \
        ...
        XML::Parser)
            ...
            LDFLAGS="$FLAGS $OSX_ARCH $OSX_FLAGS" \
                ./configure --prefix=$BUILD \
                --disable-dependency-tracking --disable-shared $CROSS_OPTS
            make
            if [ $? != 0 ]; then
                echo "make failed"
                exit $?
        ...
        Media::Scan)            
			...
			if ! [ -f $BUILD/include/libmediascan.h ]; then
				# build libmediascan
				...
				cd libmediascan-0.1
				CC="$CC" CXX="$CXX" LD="$LD"
				CFLAGS="$CFLAGS -I$BUILD/include $FLAGS $OSX_ARCH $OSX_FLAGS " \
				LDFLAGS="$LDFLAGS -L$BUILD/lib $FLAGS $OSX_ARCH $OSX_FLAGS " \
				OBJCFLAGS="-L$BUILD/lib $FLAGS $OSX_ARCH $OSX_FLAGS " \
					./configure --prefix=$BUILD --disable-dependency-tracking $CROSS_OPTS
				make
				if [ $? != 0 ]; then
					echo "make failed"
					exit $?
            ...
            if [ $PERL_BIN ]; then
                $PERL_BIN Makefile.PL $MSOPTS INSTALL_BASE=$PERL_BASE PERL_SRC="$PERL_SRC"
                make CC="$CC" CFLAGS="$CFLAGS" CCFLAGS="$CCFLAGS" CPPFLAGS="$CPPFLAGS" CXX="$CXX" CXXFLAGS="$CXXFLAGS" LD="$CC" LDFLAGS="$LDFLAGS" LDDLFLAGS="$LDDLFLAGS -L$BUILD/lib -lmediascan" LD_RUN_PATH="$LD_RUN_PATH" 
                if [ $? != 0 ]; then
                    echo "make failed, aborting"
                    exit $?
                fi
...
#                     exit $?
#                 fi
                make CC="$CC" CFLAGS="$CFLAGS" CCFLAGS="$CCFLAGS" CPPFLAGS="$CPPFLAGS" CXX="$CXX" CXXFLAGS="$CXXFLAGS" LD="$LD" LDFLAGS="$LDFLAGS" LDDLFLAGS="$LDDLFLAGS" LD_RUN_PATH="$LD_RUN_PATH" install
                #make clean
            fi
...
function build_libjpeg {
    ...
    if [ "$OS" = "Darwin" -a "$OSX_VER" != "10.5" ]; then
        ...
        # Build 32-bit fork
        make clean
        CFLAGS=" -m32 $OSX_FLAGS" \
        CXXFLAGS=" -m32 $OSX_FLAGS" \
        ...
        CFLAGS="-arch ppc  $OSX_FLAGS" \
        LDFLAGS="-arch ppc  $OSX_FLAGS" \
            ./configure --prefix=$BUILD \
            --disable-dependency-tracking
        make
        if [ $? != 0 ]; then
...
function build_ffmpeg {
    ...
    if [ $OS = "Darwin" ]; then
        ...
        if [ $OSX_VER != "10.5" ]; then
            FLAGS="-arch x86_64  -fPIC $OSX_FLAGS"      
            CFLAGS="$FLAGS" \
            LDFLAGS="$FLAGS" \
	...
	# ffmpeg-0.8.4/libavcodec/arm/jrevdct_arm.S has absulte data memory references, can't be used as shared
	# library, objdump show that TEXTREL is defined, and as as uclibc was build with FORCE_SHAREABLE_TEXT_SEGMENTS=y wich requires relocation!
	# so, disable arm assembly code, as I don't know how to fix that assembly code 
	# its still there in ffmpeg-1.2.4
        CFLAGS="$CFLAGS $FLAGS " \
        LDFLAGS="$LDFLAGS $FLAGS " \
            ./configure $FFOPTS --arch=arm --target-os=linux --enable-cross-compile --cross-prefix=${CROSS}- --cc="$CC" --disable-asm --enable-pic
...
function build_bdb {
    ...
    LDFLAGS="$FLAGS $OSX_ARCH $OSX_FLAGS " \
        ...
        --disable-dependency-tracking --disable-shared --disable-posixmutexes --disable-uimutexes -with-mutex=ARM/gcc-assembly $CROSS_OPTS
    make
    if [ $? != 0 ]; then
        echo "make failed"
        exit $?
